...
var PORTAL_CACHE_KEY_='CES_PORTAL_CACHE_';
var PORTAL_LINKS_CACHE_KEY_='CES_HOME_LINKS_CACHE';
var PORTAL_LINKS_FALLBACK_={success:true,source:'FRONTEND_FALLBACK',
applications:[
...
{id:'PL-PM-CAL-REMINDER',section:'APPLICATION',titleTh:'CAL/PM Reminder',titleEn:'CAL/PM Reminder',descriptionTh:'Calibration and PM reminder list',descriptionEn:'Calibration and PM reminder list',url:'https://bdmsgroup.sharepoint.com/sites/PM-CalReminder/Lists/PMCal%20Reminder/AllItems.aspx?id=%2Fsites%2FPM%2DCalReminder%2FLists%2FPMCal%20Reminder%2FCentral%201&viewid=094c06a0%2D09c6%2D4e3c%2D8537%2D091bf90e5977&newTargetListUrl=%2Fsites%2FPM%2DCalReminder%2FLists%2FPMCal%20Reminder&viewpath=%2Fsites%2FPM%2DCalReminder%2FLists%2FPMCal%20Reminder%2FAllItems%2Easpx&ovuser=325c40be%2D6d2b%2D4006%2Db2c5%2D078947c856d2%2CSiripak%2ECh%40BDMS%2ECO%2ETH&TeamsCID=20955382%2Daa40%2D4b25%2D899f%2D3638337417b6&OR=Teams%2DHL&CT=1787643535138&clickparams=eyJBcHBOYW1lIjoiVGVhbXMtRGVza3RvcCIsIkFwcFZlcnNpb24iOiI0OS8yNjA3MTYxNjAxNCIsIkhhc0ZlZGVyYXRlZFVzZXIiOmZhbHNlfQ%3D%3D',icon:'fa-calendar-check',theme:'blue',featured:true,sortOrder:45},
{id:'PL-CES-DRIVE',section:'APPLICATION',titleTh:'CES Drive',titleEn:'CES Drive',descriptionTh:'พื้นที่จัดเก็บและแชร์ไฟล์ CES บน Microsoft SharePoint',descriptionEn:'CES shared file storage on Microsoft SharePoint',url:'https://bdmsgroup-my.sharepoint.com/:f:/g/personal/siripak_ch_bdms_co_th/IgA07sfK6oRXTYrI4dlXPgldAR2vc00yCfh7ODreRBqiWR4?e=d9Iz8k',icon:'fa-hard-drive',theme:'blue',featured:false,sortOrder:9},{id:'PL-EBOOK',section:'APPLICATION',titleTh:'E-Book',titleEn:'E-Book',descriptionTh:'คลังเรียนรู้และเอกสารอ้างอิง',descriptionEn:'Learning and reference library',url:'https://anyflip.com/bookcase/spgdj/',icon:'fa-book-open',theme:'green',sortOrder:8},{id:'PL-PEOPLE-CONNECT',section:'APPLICATION',titleTh:'People Connect',titleEn:'People Connect',descriptionTh:'ระบบข้อมูลบุคลากรและ HR ต้องเชื่อมต่อ VPN',descriptionEn:'People and HR portal — VPN required',url:'https://hcm44.sapsf.com/sf/home?bplte_company=bangkokdus&_s.crb=Y8qBGWLX55W1e2fZoIZsjfFR79J2g88vaNfAVnyxb0w%253d',icon:'fa-users-rectangle',theme:'navy',sortOrder:9}],
nhealthServices:[
...
function userDisplayName(){var u=user();return String(u.name_eng||u.nameEng||u.Name_ENG||u.name_th||u.nameTh||u.Name_TH||u.name||u.displayName||u.id||'CES Team').trim()||'CES Team';}
function primeHomeIdentity(){setText('portal-user-name',userDisplayName());clock();return true;}
...
function mergeCanonicalPortal(incoming,defaults){
  incoming=Array.isArray(incoming)?incoming.filter(Boolean):[];defaults=Array.isArray(defaults)?defaults:[];
  if(incoming.length)return incoming.filter(function(x){return String(x&&x.status||'ACTIVE').toUpperCase()==='ACTIVE'&&String(x&&x.id||'').toUpperCase()!=='PL-FORMBRICKS'&&!/FORMBRICKS/i.test(String(x&&x.titleEn||x&&x.titleTh||''));});
  var byId={};incoming.forEach(function(x){var id=String(x&&x.id||'').trim();if(id)byId[id]=x;});
  defaults.forEach(function(def){var id=String(def&&def.id||'').trim();if(!id)return;var current=byId[id];byId[id]=current?Object.assign({},def,current,{id:id}):Object.assign({},def,{id:id});});
...
function normalizePortalLinks(data){
  data=data||{};
  ...
  apps=mergeCanonicalPortal(apps,PORTAL_LINKS_FALLBACK_.applications);
  services=mergeCanonicalPortal(services,PORTAL_LINKS_FALLBACK_.nhealthServices);
  innovations=mergeCanonicalPortal(innovations,PORTAL_LINKS_FALLBACK_.innovations);
...
function ensurePortalFallback(){
  ...
  if(!hasApps||!hasServices||!hasInnovations){
    var cached=readPortalLinksCache();
    var usable=cached&&((cached.applications||[]).length||(cached.innovations||[]).length)?cached:PORTAL_LINKS_FALLBACK_;
    state.portalLinks=normalizePortalLinks(usable);
...
function homeOverlay(show,message){
...
function dispatchHomeReady(detail){try{window.dispatchEvent(new CustomEvent('ces:home-ready',{detail:detail||{}}));}catch(ignore){}}
...
function renderPortalLinks(data){
  data=normalizePortalLinks(data);
  var apps=document.getElementById('portal-app-grid'),services=document.getElementById('portal-services-grid'),innovations=document.getElementById('portal-innovation-grid');
  var lang=(window.CES_LANGUAGE&&window.CES_LANGUAGE.get&&window.CES_LANGUAGE.get())||'EN';
  var canonical=['PL-N-SMART','PL-N-CERT','PL-IHB','PL-PM-CAL-REMINDER','PL-CES-DRIVE','PL-STEP','PL-EMEMO','PL-EBOOK','PL-PEOPLE-CONNECT'];
  ...
  if(apps){var rows=sortApps((data.applications.length?data.applications:PORTAL_LINKS_FALLBACK_.applications).slice());apps.innerHTML=rows.map(function(x){var isVpn=/\(VPN\)/i.test(String(x.titleEn||x.titleTh||''));var forceWhite=String(x.id||'').toUpperCase()==='PL-PM-CAL-REMINDER';var featured=!!x.featured&&!forceWhite;return '<a class="ces-portal-app-card '+(featured?'featured ':'')+(forceWhite?'ces-portal-app-card-white ':'')+(isVpn?'vpn-card':'')+'" href="'+esc(x.url||'#')+'" target="_blank" rel="noopener"><div class="ces-portal-app-icon '+esc(x.theme||'blue')+'"><i class="fas '+esc(x.icon||'fa-link')+'"></i></div><div><h4>'+esc(title(x))+(isVpn?'<span class="ces-portal-vpn-badge">VPN</span>':'')+'</h4><p>'+esc(desc(x))+'</p></div><i class="fas fa-arrow-up-right-from-square open"></i></a>';}).join('');apps.dataset.portalFallback=String(!data.applications.length);}
  if(services){var serviceRows=(data.nhealthServices.length?data.nhealthServices:PORTAL_LINKS_FALLBACK_.nhealthServices).slice().sort(function(a,b){return Number(a.sortOrder||999)-Number(b.sortOrder||999);});services.innerHTML=serviceRows.map(function(x){var isVpn=/VPN|IT Support|Intranet/i.test(String(x.titleEn||x.titleTh||''));return '<a class="ces-portal-app-card '+(x.featured?'featured ':'')+(isVpn?'vpn-card':'')+'" href="'+esc(x.url||'#')+'" target="_blank" rel="noopener"><div class="ces-portal-app-icon '+esc(x.theme||'blue')+'"><i class="fas '+esc(x.icon||'fa-link')+'"></i></div><div><h4>'+esc(title(x))+(isVpn?'<span class="ces-portal-vpn-badge">VPN</span>':'')+'</h4><p>'+esc(desc(x))+'</p></div><i class="fas fa-arrow-up-right-from-square open"></i></a>';}).join('');services.dataset.portalFallback=String(!data.nhealthServices.length);}
  if(innovations){var items=(data.innovations.length?data.innovations:PORTAL_LINKS_FALLBACK_.innovations).slice().sort(function(a,b){return Number(a.sortOrder||999)-Number(b.sortOrder||999);});innovations.innerHTML=items.map(function(x){var theme=String(x.theme||'').toLowerCase();if(theme==='teal')theme='smart';if(theme==='slate')theme='promed';return '<a class="ces-portal-innovation '+esc(theme||'smart')+'" href="'+esc(x.url||'#')+'" target="_blank" rel="noopener"><div class="ces-portal-tool-icon"><i class="fas '+esc(x.icon||'fa-screwdriver-wrench')+'"></i></div><div class="ces-portal-tool-copy"><span>'+esc(lang==='TH'?'เครื่องมือ CES':'CES Tool')+'</span><h4>'+esc(title(x))+'</h4><p>'+esc(desc(x))+'</p></div><i class="fas fa-arrow-right"></i></a>';}).join('');innovations.dataset.portalFallback=String(!data.innovations.length);}
}
function homeCardsReady(){
...
function renderHomeCritical(res){
  res=res||{};
  ...
  var links=normalizePortalLinks(res.portalLinks||(res.applications||res.innovations?res:{}));
  if(links.applications.length||links.innovations.length){state.portalLinks=links;renderPortalLinks(links);writePortalLinksCache(links);}
  else renderPortalLinks(state.portalLinks||PORTAL_LINKS_FALLBACK_);
  ensurePortalFallback();
  return homeCardsReady();
}
async function loadHomeCritical(force){
  primeHomeIdentity();primePortalLinks();ensurePortalFallback();
  var opt={force:!!force,userId:user().id||''};
  try{
    var res=await apiFirst(['getHomeCritical'],[opt],{timeoutMs:14000});
    if(res&&res.success!==false){renderHomeCritical(res);return res;}
  }catch(error){console.warn('[Home critical]',error);}
  try{await loadPortalLinks(!!force);}catch(ignore){}
  ensurePortalFallback();
  return {success:false,fallback:true,applications:(state.portalLinks&&state.portalLinks.applications)||PORTAL_LINKS_FALLBACK_.applications,innovations:(state.portalLinks&&state.portalLinks.innovations)||PORTAL_LINKS_FALLBACK_.innovations};
}
function readPortalLinksCache(){try{var x=JSON.parse(localStorage.getItem(PORTAL_LINKS_CACHE_KEY_)||'null');var d=x&&x.data?x.data:null;return d&&(Array.isArray(d.applications)||Array.isArray(d.innovations))?d:null;}catch(e){return null;}}
function writePortalLinksCache(data){try{localStorage.setItem(PORTAL_LINKS_CACHE_KEY_,JSON.stringify({at:Date.now(),data:data}));}catch(e){}}
function primePortalLinks(){var cached=readPortalLinksCache();var usable=cached&&((cached.applications||[]).length||(cached.innovations||[]).length)?cached:PORTAL_LINKS_FALLBACK_;state.portalLinks=normalizePortalLinks(usable);renderPortalLinks(state.portalLinks);ensurePortalFallback();return state.portalLinks;}
async function loadPortalLinks(force){
  if(!state.portalLinks)primePortalLinks();if(state.linkLoadingPromise&&!force)return state.linkLoadingPromise;
  state.linkLoadingPromise=(async function(){try{var res=await apiFirst(['getPortalLinks'],[!!force],{timeoutMs:12000});var normalized=normalizePortalLinks(res);if(normalized.success&&(normalized.applications.length||normalized.innovations.length)){state.portalLinks=normalized;renderPortalLinks(normalized);writePortalLinksCache(normalized);return normalized;}}catch(e){console.warn('[Home links]',e);}finally{state.linkLoadingPromise=null;}renderPortalLinks(state.portalLinks||PORTAL_LINKS_FALLBACK_);ensurePortalFallback();return state.portalLinks||normalizePortalLinks(PORTAL_LINKS_FALLBACK_);})();return state.linkLoadingPromise;
}
...
function portalCacheKey(){return PORTAL_CACHE_KEY_+String((user()&&user().id)||'anonymous');}
...
function renderPortalData(res){
  ...
  var links=normalizePortalLinks(res.portalLinks||(res.applications||res.innovations?res:{}));if(links.applications.length||links.innovations.length){state.portalLinks=links;renderPortalLinks(links);writePortalLinksCache(links);}else{renderPortalLinks(state.portalLinks||primePortalLinks());}
  ensurePortalFallback();if(!state.filterReady)filters(res);var add=document.getElementById('portal-add-event-btn');if(add)add.classList.toggle('hidden',!admin());
}
async function load(force){
  var root=document.getElementById('view-portal');if(!root)return null;ensurePortalFallback();
...
async function priorityBootstrap(force){
  ...
  state.priorityPromise=(async function(){
    ...
    try{
      ...
      ensurePortalFallback();state.firstReady=true;homeOverlay(false);
      dispatchHomeReady({success:false,error:String(error&&error.message||error),fallback:true});
      return state.data||(cached&&cached.data)||null;
    }finally{state.priorityPromise=null;}
  })();
  var safety=setTimeout(function(){
    if(!state.firstReady){primeHomeIdentity();ensurePortalFallback();state.firstReady=true;homeOverlay(false);dispatchHomeReady({success:false,timeout:true,fallback:true});}
  },10000);
  state.priorityPromise.finally(function(){clearTimeout(safety);});
  return state.priorityPromise;
}

function bindPortalLinks(){var grid=document.getElementById('portal-app-grid');if(!grid||grid.dataset.bound==='1')return;grid.dataset.bound='1';grid.addEventListener('click',function(event){var link=event.target.closest('a');if(!link)return;var u=user();try{if(window.CES_API&&u.id&&Date.now()-Number(state.usageAt||0)>60000){state.usageAt=Date.now();window.CES_API.callFunction('recordPortalUsage',[{employeeId:u.id,name:u.name_eng||u.name_th||'',team:u.team||'',role:u.role||'',action:'OPEN_LINK',module:'portal',source:'web'}],{transport:'jsonp',timeoutMs:8000}).catch(function(){});}}catch(ignore){}});}
async function syncHomePortal(){
  try{var btn=document.getElementById('ces-home-sync-btn');if(btn){btn.disabled=true;btn.classList.add('is-loading');}await Promise.allSettled([loadPortalLinks(true),load(true)]);}
...
function init(force){var u=user();primeHomeIdentity();ensurePortalFallback();primePortalLinks();setTimeout(function(){primeHomeIdentity();ensurePortalFallback();},50);setTimeout(function(){ensurePortalFallback();},700);bindPortalLinks();var add=document.getElementById('portal-add-event-btn');if(add)add.classList.toggle('hidden',!admin());try{if(window.CES_API&&u.id&&Date.now()-Number(state.usageAt||0)>60000){state.usageAt=Date.now();window.CES_API.callFunction('recordPortalUsage',[{employeeId:u.id,name:u.name_eng||u.name_th||'',team:u.team||'',role:u.role||'',action:'ACTIVE',module:'portal',sessionId:(typeof cesSessionId_==='function'?cesSessionId_():''),source:'web'}],{transport:'jsonp',timeoutMs:12000}).catch(function(){});}}catch(ignore){}if(force||!state.loaded||!state.firstReady)priorityBootstrap(!!force);else load(false).catch(function(){});}
window.CES_refreshPortalLinksAfterSettingSave=async function(){try{localStorage.removeItem(PORTAL_LINKS_CACHE_KEY_);localStorage.removeItem('CES_HOME_LINKS_CACHE_LEGACY');localStorage.removeItem(portalCacheKey());}catch(ignore){}state.portalLinks=null;state.loaded=false;state.firstReady=false;return loadPortalLinks(true).then(function(result){renderPortalLinks(result);return result;});};
