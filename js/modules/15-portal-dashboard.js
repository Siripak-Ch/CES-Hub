...
var PORTAL_LINKS_CACHE_KEY_V203='CES_HOME_LINKS_CACHE_V3026';
...
function normalizePortalLinks(data){
  ...
  apps=mergeCanonicalPortal(apps,PORTAL_LINKS_FALLBACK_V203.applications);
  services=mergeCanonicalPortal(services,PORTAL_LINKS_FALLBACK_V203.nhealthServices);
  innovations=mergeCanonicalPortal(innovations,PORTAL_LINKS_FALLBACK_V203.innovations);
...
function readPortalLinksCache(){try{var x=JSON.parse(localStorage.getItem(PORTAL_LINKS_CACHE_KEY_V203)||'null');var d=x&&x.data?x.data:null;return d&&(Array.isArray(d.applications)||Array.isArray(d.innovations))?d:null;}catch(e){return null;}}
function writePortalLinksCache(data){try{localStorage.setItem(PORTAL_LINKS_CACHE_KEY_V203,JSON.stringify({at:Date.now(),data:data}));}catch(e){}}
function primePortalLinks(){var cached=readPortalLinksCache();var usable=cached&&((cached.applications||[]).length||(cached.innovations||[]).length)?cached:PORTAL_LINKS_FALLBACK_V203;state.portalLinks=normalizePortalLinks(usable);renderPortalLinks(state.portalLinks);ensurePortalFallback();return state.portalLinks;}
async function loadPortalLinks(force){
  ...
  state.linkLoadingPromise=(async function(){try{var res=await apiFirst(['getPortalLinks'],[!!force],{timeoutMs:12000});var normalized=normalizePortalLinks(res);if(normalized.success&&(normalized.applications.length||normalized.innovations.length)){state.portalLinks=normalized;renderPortalLinks(normalized);writePortalLinksCache(normalized);return normalized;}}catch(e){console.warn('[Home links]',e);}finally{state.linkLoadingPromise=null;}renderPortalLinks(state.portalLinks||PORTAL_LINKS_FALLBACK_V203);ensurePortalFallback();return state.portalLinks||normalizePortalLinks(PORTAL_LINKS_FALLBACK_V203);})();return state.linkLoadingPromise;
}
...
function init(force){var u=user();primeHomeIdentity();ensurePortalFallback();primePortalLinks();setTimeout(function(){primeHomeIdentity();ensurePortalFallback();},50);setTimeout(function(){ensurePortalFallback();},700);bindPortalLinks();var add=document.getElementById('portal-add-event-btn');if(add)add.classList.toggle('hidden',!admin());try{if(window.CES_API&&u.id&&Date.now()-Number(state.usageAt||0)>60000){state.usageAt=Date.now();window.CES_API.callFunction('recordPortalUsage',[{employeeId:u.id,name:u.name_eng||u.name_th||'',team:u.team||'',role:u.role||'',action:'ACTIVE',module:'portal',sessionId:(typeof cesSessionIdV50_==='function'?cesSessionIdV50_():''),source:'web'}],{transport:'jsonp',timeoutMs:12000}).catch(function(){});}}catch(ignore){}if(force||!state.loaded||!state.firstReady)priorityBootstrap(!!force);else load(false).catch(function(){});}
window.CES_refreshPortalLinksAfterSettingSave=async function(){try{localStorage.removeItem(PORTAL_LINKS_CACHE_KEY_V203);localStorage.removeItem('CES_HOME_LINKS_CACHE_V3012');localStorage.removeItem(portalCacheKey());}catch(ignore){}state.portalLinks=null;state.loaded=false;state.firstReady=false;return loadPortalLinks(true).then(function(result){renderPortalLinks(result);return result;});};
