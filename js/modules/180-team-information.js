...
function cesTeamColor_(team){
  const colors={MED:['#eff6ff','#1d4ed8','#bfdbfe'],LAB:['#ecfeff','#0e7490','#a5f3fc'],EHS:['#ecfdf5','#047857','#a7f3d0'],ENV:['#f0fdf4','#15803d','#bbf7d0'],TES:['#fffbeb','#b45309','#fde68a'],QM:['#faf5ff','#7e22ce','#e9d5ff'],MNG:['#f8fafc','#334155','#cbd5e1'],SALES:['#fff1f2','#be123c','#fecdd3'],REPORT:['#eef2ff','#4338ca','#c7d2fe'],OTHER:['#fff7ed','#c2410c','#fed7aa']},c=colors[String(team||'OTHER').toUpperCase()]||colors.OTHER;
  return {bg:c[0],text:c[1],border:c[2],contrast:'#0f172a'};
}
function cesTeamNames_(rows){var seen={},out=[];(rows||[]).forEach(function(r){var t=String(r&&r.team||'OTHER').trim().toUpperCase()||'OTHER';if(!seen[t]){seen[t]=1;out.push(t);}});return out.sort(function(a,b){return a==='OTHER'?1:b==='OTHER'?-1:a.localeCompare(b);});}
...
function renderTeamTabs_(){
  const teams=['ALL'].concat(cesTeamNames_(CES_TEAM_INFO.rows)),root=document.getElementById('team-info-tabs');if(!root)return;
...
function renderTeamInformation(){
  ...
  const filtered=rows.filter(r=>{const team=String(r.team||'OTHER').trim().toUpperCase()||'OTHER';const matchTeam=CES_TEAM_INFO.team==='ALL'||team===CES_TEAM_INFO.team;const text=[r.id,r.nameTh,r.nameEng,r.email,r.team,r.position,r.supervisor,r.tel,r.costCenter].join(' ').toLowerCase();return matchTeam&&(!q||text.includes(q));});
...
function renderTrainingDashboard(){
  ...
  const teamFilter=document.getElementById('training-team-filter');if(teamFilter){const selected=teamFilter.value||'ALL',names=cesTeamNames_((data.people||[]).map(function(p){return{team:p.team};}));teamFilter.innerHTML=['ALL'].concat(names).map(function(t){return'<option value="'+cesTeamEsc_(t)+'" '+(t===selected?'selected':'')+'>'+(t==='ALL'?'All Teams':cesTeamEsc_(t))+'</option>';}).join('');if(!['ALL'].concat(names).includes(selected))teamFilter.value='ALL';}
...
function cesTrainingExcelDate(value){
...
function cesTrainingHours(value){
...
async function importTrainingWorkbook(file){
  ...
  try{
    ...
    const records=rows.map((r,index)=>({
      ...
      startDate:cesTrainingExcelDate(r['Start Date']),endDate:cesTrainingExcelDate(r['End Date']),completedStatus:String(r['Completed Status']||'').trim(),completedDate:cesTrainingExcelDate(r['Completed Date']),totalHour:cesTrainingHours(r['Total Hour'])
