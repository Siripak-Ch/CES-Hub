...
function renderTeamInformation(){
  ...
  root.innerHTML=`<div class="ces-team-info-table-wrap overflow-auto rounded-2xl border border-slate-200 max-h-[680px]"><table class="ces-team-info-table w-full min-w-[1280px] text-xs text-left"><thead class="sticky top-0 z-20 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500"><tr><th class="p-3 sticky left-0 z-30 bg-slate-50 min-w-[250px]">Staff</th><th class="p-3 min-w-[90px]">Team</th><th class="p-3 min-w-[260px]">Position / Role</th><th class="p-3 min-w-[250px]">Email</th><th class="p-3 min-w-[190px]">Supervisor</th><th class="p-3 min-w-[150px]">Employee Type</th><th class="p-3 min-w-[140px]">Contact</th>${admin?'<th class="p-3 text-center min-w-[80px]">Action</th>':''}</tr></thead><tbody class="divide-y divide-slate-100">${filtered.map(r=>{const rawTeam=String(r.team||'').toUpperCase();const displayTeam=['MED','LAB','EHS','ENV','TES','QM','MNG','SALES'].includes(rawTeam)?rawTeam:'MNG';const c=cesTeamColor_(displayTeam);return `<tr class="hover:bg-slate-50/80" style="border-left:4px solid ${c.text}"><td class="p-3 sticky left-0 bg-white group-hover:bg-slate-50 z-10"><b class="text-slate-800 whitespace-nowrap">${cesTeamEsc_(r.nameEng||r.nameTh||r.id)}</b><div class="text-[10px] text-slate-400 whitespace-nowrap">${cesTeamEsc_(r.nameTh||'')} · ${cesTeamEsc_(r.id||'')}</div></td><td class="p-3"><span class="inline-flex px-2.5 py-1 rounded-full font-black whitespace-nowrap" style="background:${c.bg};color:${c.text};border:1px solid ${c.border}">${cesTeamEsc_(displayTeam)}</span></td><td class="p-3 font-bold text-slate-600"><div class="leading-5">${cesTeamEsc_(r.position||'-')}</div><div class="text-[10px] text-slate-400 mt-1 whitespace-nowrap">${cesTeamEsc_(r.role||'-')}</div></td><td class="p-3 text-slate-600 whitespace-nowrap"><a class="hover:underline" href="mailto:${cesTeamEsc_(r.email||'')}">${cesTeamEsc_(r.email||'-')}</a></td><td class="p-3 text-slate-600 whitespace-nowrap">${cesTeamEsc_(r.supervisor||'-')}</td><td class="p-3 text-slate-600 whitespace-nowrap">${cesTeamEsc_(r.empType||'-')}</td><td class="p-3 text-slate-600 whitespace-nowrap">${cesTeamEsc_(r.tel||'-')}</td>${admin?`<td class="p-3 text-center"><button class="w-8 h-8 rounded-lg bg-blue-50 text-[#003DA5] border border-blue-100" onclick='editTeamInformation(${JSON.stringify(r).replace(/'/g,"&#39;")})' title="Edit"><i class="fas fa-pen"></i></button></td>`:''}</tr>`;}).join('')}</tbody></table></div><div class="mt-3 text-[10px] font-bold text-slate-400">${admin?'Admin edit enabled':'Read-only'} · ${filtered.length} staff · Scroll horizontally to view all columns</div>`;
}
