...
function initTeamInformation(){
  ...
  if(cached){
    CES_TEAM_INFO.loaded=true;CES_TEAM_INFO.rows=cached.data;CES_TEAM_INFO.generatedAt=cached.generatedAt||'';CES_TEAM_INFO.cacheSource='browser';teamTabs=Array.from(new Set(['ALL','REPORT'].concat((CES_TEAM_INFO.rows||[]).map(r=>String(r.team||'OTHER').toUpperCase()).filter(Boolean))));
...
function loadTeamInformation(force,background){
  ...
  const ok=res=>{
    ...
    CES_TEAM_INFO.loaded=true;CES_TEAM_INFO.rows=res.data||[];CES_TEAM_INFO.generatedAt=res.generatedAt||'';CES_TEAM_INFO.cacheSource='sheet';teamTabs=Array.from(new Set(['ALL','REPORT'].concat((CES_TEAM_INFO.rows||[]).map(r=>String(r.team||'OTHER').toUpperCase()).filter(Boolean))));
...
function renderTeamTabs_(){
  const teams=teamTabs,root=document.getElementById('team-info-tabs');if(!root)return;
...
function renderTeamInformation(){
  ...
  const filtered=rows.filter(r=>{const rawTeam=(r.team||'').toUpperCase();const team=teamTabs.filter(function(t){return t!=='ALL';}).includes(rawTeam)?rawTeam:'OTHER';const matchTeam=CES_TEAM_INFO.team==='ALL'||team===CES_TEAM_INFO.team;const text=[r.id,r.nameTh,r.nameEng,r.email,r.team,r.position,r.supervisor,r.tel,r.costCenter].join(' ').toLowerCase();return matchTeam&&(!q||text.includes(q));});
