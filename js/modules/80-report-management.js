...
function fillUserInfoRM() {
    ...
    function openTEGenerate() {
        if (!window.Swal) return;
        var now=new Date(),user=(typeof currentUser!=='undefined'&&currentUser)||{};
        Swal.fire({title:'TE Generate · TE_Info',width:680,html:'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;text-align:left"><label>Employee ID<input id="teGenEmp" class="swal2-input" value="'+rmEscapeHtml(user.id||user.empId||'')+'"></label><label>Employee name<input id="teGenName" class="swal2-input" value="'+rmEscapeHtml(user.name_th||user.name||'')+'"></label><label>Year<input id="teGenYear" class="swal2-input" type="number" value="'+now.getFullYear()+'"></label><label>Month<select id="teGenMonth" class="swal2-input">'+Array.from({length:12},function(_,i){var n=i+1;return '<option value="'+n+'" '+(n===now.getMonth()+1?'selected':'')+'>'+new Date(2000,i,1).toLocaleString('en',{month:'long'})+'</option>';}).join('')+'</select></label></div><p style="font-size:11px;color:#64748b;text-align:left;margin:12px 4px 0">Reads TE_Info only; the source sheet is never changed. Attachments are grouped by date in TE_Attachments.</p>',showCancelButton:true,confirmButtonText:'Check & Generate',confirmButtonColor:'#059669',preConfirm:function(){return{employeeId:document.getElementById('teGenEmp').value.trim(),employeeName:document.getElementById('teGenName').value.trim(),year:Number(document.getElementById('teGenYear').value),month:Number(document.getElementById('teGenMonth').value)};}}).then(function(answer){if(!answer.isConfirmed)return;var p=answer.value;Swal.fire({title:'Checking TE_Info…',allowOutsideClick:false,showConfirmButton:false,didOpen:function(){Swal.showLoading();}});window.CES_API.callFunction('getTEGenerateData',[p],{transport:'jsonp',timeoutMs:30000,dedupe:false,priority:'active',module:'report_manage'}).then(function(preview){if(!preview||preview.success===false)throw new Error(preview&&preview.message||'TE_Info unavailable');if(!preview.rows.length)throw new Error('No TE_Info rows match the selected period / employee.');return Swal.fire({icon:'question',title:'Generate TE?',html:'<b>'+preview.rows.length+'</b> rows · total ฿'+Number(preview.total||0).toLocaleString('en-US',{maximumFractionDigits:2})+' · '+Number(preview.attachmentCount||0)+' attachment link(s)',showCancelButton:true,confirmButtonText:'Generate PDF + XLSX',confirmButtonColor:'#059669'}).then(function(ok){if(!ok.isConfirmed)return null;Swal.fire({title:'Generating TE…',text:'Creating document and attachment index',allowOutsideClick:false,showConfirmButton:false,didOpen:function(){Swal.showLoading();}});return window.CES_API.callFunction('generateTEReport',[p],{transport:'iframe',timeoutMs:180000,dedupe:false,priority:'active',userAction:true,module:'report_manage'});});}).then(function(res){if(!res)return;if(res.success===false)throw new Error(res.message||'TE generation failed');Swal.fire({icon:'success',title:'TE generated',html:'<p>'+Number(res.recordCount||0)+' rows · ฿'+Number(res.total||0).toLocaleString('en-US',{maximumFractionDigits:2})+'</p><p style="margin-top:12px"><a href="'+rmEscapeHtml(res.pdfUrl)+'" target="_blank">Open PDF</a> · <a href="'+rmEscapeHtml(res.xlsxUrl)+'" target="_blank">Open XLSX</a></p>'});}).catch(function(e){Swal.fire('TE Generate',rmErrorMessage(e),'error');});});
    }
    window.openTEGenerate=openTEGenerate;
