...
function renderServiceMemoMappingV55_(){
  ...
  if(memoInline){memoInline.innerHTML='<div class="ces-service-memo-inline-v266">'+svcMapV55PopupHtml_()+'</div>';return;}
...
function svcMapV55Recent3MonthKeys_(){const now=new Date(),keys=[];for(let i=0;i<3;i++){const d=new Date(now.getFullYear(),now.getMonth()-i,1);keys.push(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`);}return new Set(keys);}
async function handleServiceMemoMapping(event){
  ...
  try{
    ...
    if(!memoRows.length&&!workOrderRows.length)throw new Error('No dated 2026 rows were found in Memo or WorkOrder.');const allMappingRows=svcBuildMemoMappingV55_(memoRows,workOrderRows);if(!allMappingRows.length)throw new Error('No comparison rows could be created.');const recent3=svcMapV55Recent3MonthKeys_();const mappingRows=allMappingRows.filter(row=>recent3.has(svcMapV55RowMonth_(row)));if(!mappingRows.length)throw new Error('No comparison rows were found in the latest 3 months.');
