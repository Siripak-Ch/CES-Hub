...
function wkSetInitialLoading_(visible,message,isError){
    ...
    if(isError){loading.innerHTML='<div class="text-center px-6"><i class="fas fa-cloud-arrow-down text-3xl text-amber-500 mb-3"></i><p class="text-sm font-black text-slate-700">เชื่อมต่อข้อมูล Weekly ชั่วคราวไม่ได้</p><p class="text-xs font-bold text-slate-400 mt-1">ระบบจะเชื่อมต่อใหม่อัตโนมัติ หรือกด Retry</p><button type="button" class="mt-4 px-4 py-2 rounded-xl bg-[#003DA5] text-white text-xs font-black" onclick="refreshWeeklyInitial()"><i class="fas fa-rotate-right mr-1"></i> Retry</button></div>';return;}
...
function initWeekly(forceRefresh) {
    ...
    const display = document.getElementById('wk-display-week'); if (display) display.innerText = wkCurrentWeekInfo;
...
function switchWeeklyView(mode) {
    ...
    if (btnForm) btnForm.className = 'ces-segmented-btn' + (isForm ? ' active' : '');
    if (btnDash) btnDash.className = 'ces-segmented-btn' + (!isForm ? ' active' : '');
    if (isForm) {
...
function switchWeeklyTeam(team) {
    ...
    ['MED', 'LAB', 'EHS'].forEach(t => {
        ...
        if(t === team) {
            const style = typeof window.cesGetTeamStyle === 'function' ? window.cesGetTeamStyle(t) : {color:'#004aad',text:'#fff'};
            btn.className = 'flex-1 py-3 px-4 rounded-xl text-sm font-bold shadow-md transform scale-105 transition-all whitespace-nowrap';
            btn.style.background = style.color; btn.style.color = style.text;
...
function filterDash(team) {
    ...
    ['ALL', 'MED', 'LAB', 'EHS'].forEach(t => {
        ...
        if(btn) {
            btn.className = "px-5 py-2 rounded-lg text-sm font-bold bg-white text-gray-500 border border-transparent transition-all whitespace-nowrap";
            btn.style.background='';btn.style.color='';
            if (t === team) {
                const style=t==='ALL'?{color:'#475569',text:'#fff'}:(typeof window.cesGetTeamStyle==='function'?window.cesGetTeamStyle(t):{color:'#004aad',text:'#fff'});
                btn.className = 'px-5 py-2 rounded-lg text-sm font-bold shadow-sm transition-all whitespace-nowrap';
                btn.style.background=style.color;btn.style.color=style.text;
            }
        }
    }); applyFilters();
