...
(function (window, document) {
  ...
  function labRenderMonthlyPerformance_(rows){
    ...
    function monthLabel(k){var names=['January','February','March','April','May','June','July','August','September','October','November','December'],m=String(k||'').match(/^(\d{4})-(\d{2})$/);return m?(names[Number(m[2])-1]+' '+m[1]):k;}
    var keys=Object.keys(map).sort();host.innerHTML='<table class="w-full text-xs"><thead><tr class="bg-slate-50"><th class="p-2 text-left">Month</th><th class="p-2 text-right">Total</th><th class="p-2 text-right text-green-700">อยู่ใน KPI</th><th class="p-2 text-right text-green-700">อยู่ใน KPI %</th><th class="p-2 text-right text-red-600">เกิน KPI</th><th class="p-2 text-right text-red-600">เกิน KPI %</th></tr></thead><tbody>'+keys.map(function(k){var x=map[k],t=x.within+x.late,wp=t?x.within*100/t:0,lp=t?x.late*100/t:0;return '<tr class="border-t"><td class="p-2 font-bold">'+esc(monthLabel(k))+'</td><td class="p-2 text-right">'+t+'</td><td class="p-2 text-right font-bold text-green-700">'+x.within+'</td><td class="p-2 text-right">'+wp.toFixed(1)+'%</td><td class="p-2 text-right font-bold text-red-600">'+x.late+'</td><td class="p-2 text-right">'+lp.toFixed(1)+'%</td></tr>';}).join('')+'</tbody></table>';
  }
