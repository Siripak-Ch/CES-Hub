...
============================================================ */
let SI = { loaded:false, tab:'equip', raw:null, inv:[], acc:[], filtered:[], accFiltered:[], page:1, accPage:1, pageSize:30, cart:[] };
function initStockInventoryModule(force=false){
  ...
  const fail=err=>{const message=(err&&err.message)||String(err||'Cannot load inventory');console.error('[Inventory load]',message);spSetHtml('siTable',`<div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-800"><b>Inventory temporarily unavailable</b><div class="mt-1">${spEsc(message)}</div><button class="mt-3 px-3 py-2 rounded-lg bg-white border border-amber-200 font-bold" onclick="initStockInventoryModule(true)">Retry</button></div>`);};
  google.script.run.withSuccessHandler(res=>{if(!res||!res.success){fail((res&&res.message)||'Cannot load inventory');return;}SI.loaded=true;SI.raw=res;SI.inv=res.inventory||[];SI.acc=res.accessories||[];si_fillFilters();si_renderKpi();si_applyFilters();}).withFailureHandler(fail).si_getStockInventoryData(force===true||!SI.loaded);
}
...
function si_equipmentButtons(d){return `<div class="sp-action-group"><button class="sp-icon-btn" title="Edit" onclick='si_editPrompt(${si_json(d)})'><i class="fas fa-pen-to-square"></i></button><button class="sp-icon-btn gray" title="Mark Broken" onclick='si_markBrokenPrompt(${si_json(d)})'><i class="fas fa-screwdriver-wrench"></i></button><button class="sp-icon-btn red" title="Delete / Hide Equipment" aria-label="Delete / Hide Equipment" onclick='si_deletePrompt(${si_json(d)})'><i class="fas fa-trash"></i></button></div>`;}
function si_json(obj){return JSON.stringify(obj).replace(/'/g,'&#39;').replace(/</g,'&lt;');}
function si_renderPagination(){const total=Math.max(1,Math.ceil(SI.filtered.length/SI.pageSize));let btns='';const from=Math.max(1,SI.page-2),to=Math.min(total,SI.page+2);for(let p=from;p<=to;p++)btns+=`<button class="${p===SI.page?'active':''}" onclick="SI.page=${p};si_renderTable()">${p}</button>`;spSetHtml('siPagination',`<div class="sp-muted">Page ${SI.page} / ${total} • 30 rows per page</div><div class="sp-page-buttons"><button ${SI.page<=1?'disabled':''} onclick="SI.page--;si_renderTable()">Prev</button>${btns}<button ${SI.page>=total?'disabled':''} onclick="SI.page++;si_renderTable()">Next</button></div>`);}
...
function si_renderKpi(){
  ...
  const accTotal=SI.acc.length, accStock=SI.acc.reduce((s,a)=>s+Number(a.stockQty||a.qty||0),0), low=SI.acc.filter(a=>Number(a.stockQty||a.qty||0)<=Number(a.minStockQty||a.minStock||0)).length, cost=SI.acc.reduce((s,a)=>s+Number(a.cost||a.unitCost||a.unit_cost||a.totalCost||0),0);
  const acc=[['ACCESSORIES TYPE',accTotal,'fa-plug','#0ea5e9','#e0f2fe'],['TOTAL STOCK',accStock,'fa-boxes-stacked','#059669','#d1fae5'],['LOW STOCK',low,'fa-battery-quarter','#ea580c','#ffedd5'],['COST',cost,'fa-coins','#7c3aed','#ede9fe']];
...
if(!window._siInitPatch){
  window._siInitPatch=true;
...
if(!window._siUpdateCartPatch && typeof si_updateCart==='function'){
  window._siUpdateCartPatch=true;
...
if(!window._siOpenCartPatch && typeof si_openCart==='function'){
  window._siOpenCartPatch=true;
...
if(!window._siSubmitCheckoutPatch && typeof si_submitCheckout === 'function'){
  window._siSubmitCheckoutPatch = true;
...
/* ============================================================
   CES Stock Pro V31 — Pending Approval UI Patch
...
function siApprovalStyle(){
  ...
  st.textContent=`
    .si-v31-alert-tabs{display:flex;gap:8px;margin-bottom:14px;justify-content:center;flex-wrap:wrap}
    .si-v31-alert-tabs button{border:1px solid #dbeafe;background:#fff;color:#2563eb;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:900;cursor:pointer}
    .si-v31-alert-tabs button.active{background:#2563eb;color:#fff;border-color:#2563eb}
    .si-v31-list{display:flex;flex-direction:column;gap:10px;max-height:62vh;overflow:auto;text-align:left;padding-right:3px}
    .si-v31-row{display:grid;grid-template-columns:34px 1fr auto;gap:12px;align-items:center;border:1px solid #e5e7eb;background:#fff;border-radius:16px;padding:12px}
    .si-v31-dot{width:32px;height:32px;border-radius:12px;display:flex;align-items:center;justify-content:center;background:#eff6ff;color:#2563eb}
    .si-v31-dot.pending{background:#fff7ed;color:#d97706}.si-v31-dot.low{background:#fee2e2;color:#dc2626}
    .si-v31-title{font-size:13px;font-weight:1000;color:#0f172a}.si-v31-sub{font-size:11px;color:#64748b;margin-top:3px;line-height:1.45}
    .si-v31-actions{display:flex;gap:6px;align-items:center;flex-wrap:wrap;justify-content:flex-end}.si-v31-mini{border:0;border-radius:10px;padding:8px 10px;font-size:12px;font-weight:900;cursor:pointer}.si-v31-mini.ok{background:#059669;color:#fff}.si-v31-mini.bad{background:#ef4444;color:#fff}.si-v31-mini.soft{background:#eff6ff;color:#2563eb;border:1px solid #bfdbfe}
    @media(max-width:720px){.si-v31-row{grid-template-columns:30px 1fr}.si-v31-actions{grid-column:1/-1;justify-content:flex-start}}
  `;
  document.head.appendChild(st);
}
...
if(!window._siRenderKpiPatch && typeof si_renderKpi === 'function'){
  window._siRenderKpiPatch = true;
...
function siLowStockHtml(rows){
  ...
  return `<div class="si-v31-list">${rows.map(a=>{
    ...
    return `<div class="si-v31-row"><div class="si-v31-dot low"><i class="fas fa-battery-quarter"></i></div><div><div class="si-v31-title">${spEsc(a.itemName||a.item_name||a.name||'-')}</div><div class="si-v31-sub">${spEsc(a.team||'-')} • ${spEsc(a.accessoryId||a.accessory_id||a.idCode||'-')} • Stock ${spNum(qty)} / Min ${spNum(min)}</div></div><div class="si-v31-actions"><button class="si-v31-mini soft" onclick='si_restockPrompt(${si_json(a)})'><i class="fas fa-box-open"></i> Restock</button></div></div>`;
  }).join('')}</div>`;
}
...
function si_openAccessoryLowStockAlerts(){
  ...
  google.script.run.withSuccessHandler(res=>{
    ...
    window.siAlertTab=function(tab){
      ...
      const btn=document.getElementById('siTab_'+tab); if(btn)btn.classList.add('active');
      const body=document.getElementById('siAlertBody'); if(!body)return;
      body.innerHTML = tab==='pending' ? siPendingHtml(pending) : siLowStockHtml(low);
    };
    const html=`<div class="si-v31-alert-tabs"><button id="siTab_pending" class="active" onclick="siAlertTab('pending')"><i class="fas fa-clock"></i> Pending Approval (${pending.length})</button><button id="siTab_low" onclick="siAlertTab('low')"><i class="fas fa-battery-quarter"></i> Low Stock (${low.length})</button></div><div id="siAlertBody">${siPendingHtml(pending)}</div>`;
...
function siRejectFromAlert(requestId){
  const user=siCurrentUserPayload();
  Swal.fire({title:'Reject request?',html:`<input id="siRejectNote" class="swal2-input" placeholder="Reason / note">`,icon:'warning',showCancelButton:true,confirmButtonText:'Reject',confirmButtonColor:'#ef4444'}).then(r=>{
    if(!r.isConfirmed)return;
    ...
    }).withFailureHandler(e=>Swal.fire('Reject Error',e.message||String(e),'error')).si_rejectAccessoryRequestFromWeb({requestId,note:spVal('siRejectNote','Rejected from notification'),approverEmail:user.email,approver:user.name});
  });
}

if(!window._siInitPatch && typeof initStockInventoryModule === 'function'){
  window._siInitPatch=true;
...
function siRenderAccessoryKpi(rows, pendingOverride){
  ...
  const accItems = [
    ...
    ['Teams', k.accTeams, 'fa-users', '#7c3aed', '#ede9fe']
  ];
...
if(!window._siApplyFilterPatch){
  window._siApplyFilterPatch = true;
...
(function(){
  ...
  window.si_openAccessoryLowStockAlerts=function(){
    ...
    google.script.run.withSuccessHandler(res=>{
      ...
      const html=`<div class="si-v31-note">Alert แสดงแบบ table เพื่อลด DOM/card จำนวนมากและทำงานเร็วขึ้น</div><div class="si-v31-tabs"><button id="siTab_due" class="active" onclick="siAlertTabLegacy('due')"><i class="fas fa-calendar-times"></i> Check Due (${due.length})</button><button id="siTab_low" onclick="siAlertTabLegacy('low')"><i class="fas fa-battery-quarter"></i> Restock / Low (${low.length})</button><button id="siTab_pending" onclick="siAlertTabLegacy('pending')"><i class="fas fa-clock"></i> Pending (${pending.length})</button><button id="siTab_soon" onclick="siAlertTabLegacy('soon')"><i class="fas fa-hourglass-half"></i> Due Soon (${soon.length})</button></div><div id="siAlertBody">${renderAlertTable(due,'due')}</div>`;
...
(function(){
  ...
  function renderActionTable(rows, mode){
    ...
    const body = rows.map((a,i)=>{
      ...
      const chk = canBatch ? `<td class="chk"><input type="checkbox" class="siChk" data-idx="${i}" ${mode==='action'?'checked':''}></td>` : '';
      const qtyInput = canBatch ? `<input class="si-v32-qty siQty" data-idx="${i}" type="number" min="0" step="1" value="${stockQty(a)}">` : num(stockQty(a));
  ...
  window.siToggleAllAlertRows = function(checked){ document.querySelectorAll('.siChk').forEach(x=>{ x.checked=!!checked; }); };

  window.siBatchRecheckFromModal = function(){
    const rows = window.siAlertRows || [];
    const items = [];
    document.querySelectorAll('.siChk:checked').forEach(chk=>{
      const i = Number(chk.getAttribute('data-idx'));
      const a = rows[i]; if(!a) return;
      const qtyEl = document.querySelector(`.siQty[data-idx="${i}"]`);
  ...
  function showAlerts(alerts){
    ...
    window.siAlertTab = function(tab){
      ...
      const btn=document.getElementById('siTabLegacy_'+tab); if(btn) btn.classList.add('active');
      const body=document.getElementById('siAlertBodyLegacy'); if(!body) return;
      if(tab==='soon') body.innerHTML=renderActionTable(soon,'soon');
      else body.innerHTML=renderActionTable(action,'action');
    };
    const html=`<div class="si-v32-note">รวม Check due + Restock/Low และเลือกหลายรายการเพื่อ Recheck พร้อมกันได้</div><div class="si-v32-tabs"><button id="siTab_action_legacy" class="active" onclick="siAlertTab('action')"><i class="fas fa-triangle-exclamation"></i> Action Required (${action.length})</button><button id="siTab_soon_legacy" onclick="siAlertTab('soon')"><i class="fas fa-hourglass-half"></i> Due Soon (${soon.length})</button></div><div id="siAlertBodyLegacy">${renderActionTable(action,'action')}</div>`;
    Swal.fire({title:'Accessories Alerts',width:1200,html,confirmButtonText:'Close'});
    setHtml('siLowStockHeaderCount', action.length);
  }
  ...
  window.si_openInventoryHistory = function(){
    ...
    if(pre && pre.success !== false) return show(pre);
    Swal.fire({title:'Loading History...',allowOutsideClick:false,showConfirmButton:false,didOpen:()=>Swal.showLoading()});
...
(function(){
  ...
  function ensureLinks(){
    ...
    var old=document.getElementById('siQuickLinks');if(old)old.remove();
  ...
  if(typeof oldInit==='function'&&!window._siInit){
    ...
    window.initStockInventoryModule=function(force){
      invStyle();var r=oldInit.apply(this,arguments);setTimeout(ensureLinks,50);return r;
    };
  }
  invStyle();setTimeout(ensureLinks,100);
})();

(function(){if(document.getElementById('ces-v35-inv-style'))return;var st=document.createElement('style');st.id='ces-v35-inv-style';st.textContent='#view-inventory button:not([disabled])[class*="primary"],#view-inventory .sp-btn.primary,#view-inventory .acc-mini.green,#view-inventory .acc-mini.orange,#view-inventory .acc-mini.blue,#view-inventory .acc-mini.red{color:#fff!important}#view-inventory button:not([disabled]) i{color:inherit!important}@media(max-width:600px){#view-inventory .si-header-description,#view-inventory .si-subtitle,#view-inventory .inventory-description{display:none!important}}';document.head.appendChild(st)})();
