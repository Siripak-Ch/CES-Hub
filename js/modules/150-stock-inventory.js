...
function initStockInventoryModule(force=false){
  ...
  google.script.run.withSuccessHandler(res=>{if(!res||!res.success){fail((res&&res.message)||'Cannot load inventory');return;}SI.loaded=true;SI.raw=res;SI.inv=res.inventory||[];SI.acc=res.accessories||[];si_fillFilters();si_renderKpi();si_applyFilters();}).withFailureHandler(fail).si_getStockInventoryData(force===true);
}
...
/* CES Hub V30.0.32 — Inventory dashboard cleanup. Quick module cards are intentionally removed. */
(function(){
  'use strict';
  function cleanup(){var q=document.getElementById('siV3031QuickLinks');if(q)q.remove();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',cleanup);else cleanup();
  setTimeout(cleanup,200);setTimeout(cleanup,800);
})();
