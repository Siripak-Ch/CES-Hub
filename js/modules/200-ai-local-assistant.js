...
(function () {
  ...
  function friendlyTabName(tab) {
    var names={portal:'หน้า Home',management_overview:'Management Overview',yearly:'Job Dashboard',revenue:'Revenue Dashboard',ot:'OT Dashboard',service:'แบบประเมินการใช้บริการ',report:'แบบประเมินรายงานผล',calendar:'Master Calendar',checkin:'Check-in',car_booking:'Car Booking',van_booking:'Van Booking',weekly:'Weekly Report',kpi:'KPI Tracking',report_manage:'OT Generate',te_generate:'TE Generate',stock_dashboard:'Infusion Pump Dashboard',inventory:'Inventory',check_stock:'Check Stock',team_information:'Team Information',team_plan:'Team Plan',master_cal_pm_plan:'Master CAL/PM Plan',monthly_report:'Monthly Report',users:'User Management',ces_evaluation:'CES Hub Evaluation',ces_ai_knowledge:'CES AI Knowledge',health:'System Health',setting:'Setting'};
  ...
  function bindElements() {
    ...
    el.launcher = document.getElementById('ces-ai-local-launcher'); el.hide = document.getElementById('ces-ai-local-hide'); el.panel = document.getElementById('ces-ai-local-panel');
  ...
  function init() {
    ...
    el.launcher.addEventListener('click', function () {
      if(el.root.classList.contains('ces-ai-user-collapsed')){
        el.root.classList.remove('ces-ai-user-collapsed');
        try{localStorage.removeItem('CES_AI_USER_COLLAPSED');}catch(ignore){}
        openPanel();
        return;
      }
      el.panel.classList.contains('hidden') ? openPanel() : closePanel();
    });
    if(el.hide)el.hide.addEventListener('click',function(){closePanel();el.root.classList.add('ces-ai-user-collapsed');try{localStorage.setItem('CES_AI_USER_COLLAPSED','1');}catch(ignore){}});
    try{if(localStorage.getItem('CES_AI_USER_COLLAPSED')==='1')el.root.classList.add('ces-ai-user-collapsed');}catch(ignore){}
