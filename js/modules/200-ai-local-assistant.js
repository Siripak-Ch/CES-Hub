...
(function(){var w=window,d=document,KEY='CES_AI_LOCAL_HIDDEN';function setHidden(h){var root=d.getElementById('ces-ai-local-root');if(!root)return;try{localStorage.setItem(KEY,h?'1':'0')}catch(e){}root.classList.toggle('ces-ai-user-hidden',!!h);}function init(){var root=d.getElementById('ces-ai-local-root'),btn=d.getElementById('ces-ai-local-close'),launcher=d.getElementById('ces-ai-local-launcher');if(!root)return;var hidden=false;try{hidden=localStorage.getItem(KEY)==='1'}catch(e){}root.classList.toggle('ces-ai-user-hidden',hidden);if(btn&&!btn.__hideBound){btn.__hideBound=true;btn.addEventListener('click',function(){setHidden(true);});}if(!launcher||launcher.__reopenBound)return;launcher.__reopenBound=true;var reopen=d.createElement('button');reopen.type='button';reopen.className='ces-ai-reopen-mini';reopen.textContent='AI CES';reopen.title='Show AI CES';reopen.onclick=function(){setHidden(false);};root.parentNode&&root.parentNode.appendChild(reopen);if(!hidden)reopen.classList.add('hidden');Object.defineProperty(root,'__aiReopen',{value:reopen});setInterval(function(){reopen.classList.toggle('hidden',!root.classList.contains('ces-ai-user-hidden'));},300);}if(d.readyState==='loading')d.addEventListener('DOMContentLoaded',init);else setTimeout(init,0);var st=d.createElement('style');st.textContent='.ces-ai-local-root.ces-ai-user-hidden{display:none!important}.ces-ai-reopen-mini{position:fixed;right:18px;bottom:18px;z-index:2147483000;border:0;border-radius:999px;background:#004aad;color:#fff;padding:9px 13px;font-size:10px;font-weight:900;box-shadow:0 8px 24px rgba(0,74,173,.25)}.ces-ai-reopen-mini.hidden{display:none!important}@media(max-width:700px){.ces-ai-reopen-mini{right:12px;bottom:12px}}';d.head.appendChild(st);})();
