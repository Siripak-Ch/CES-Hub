...
function formatRevenueCurrencyShort(num) {
  num = Number(num || 0);
...
// Compatibility: V28 runtime ultimately exposed the Revenue formatter globally.
function formatCurrencyShort(num) { return formatRevenueCurrencyShort(num); }
...
function updateRevenueCards(sum) {
  ...
  setTxt('rev-total-act', formatRevenueCurrencyShort(sum.totalActual));
  setTxt('rev-total-tgt', formatRevenueCurrencyShort(sum.totalTarget));
  ...
  const updateTeamCard = function (team, data) {
    ...
    setTxt('rev-' + team + '-act', formatRevenueCurrencyShort(data.act));
    setTxt('rev-' + team + '-tgt', formatRevenueCurrencyShort(data.tgt));
...
function renderRevenueTables(list) {
  const renderTableSafe = function (id, key, hexColor) {
    ...
    tbody.innerHTML = list.map(function (d) {
      ...
      return '<tr class="ces-revenue-edit-row hover:bg-blue-50/60 transition-colors cursor-pointer" role="button" tabindex="0" title="Edit ' + key.toUpperCase() + ' revenue for ' + d.month + '" onclick="openRevenueEditModal(\'' + key + '\',\'' + d.month + '\')" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openRevenueEditModal(\'' + key + '\',\'' + d.month + '\')}">' +
        ...
        '<td class="p-3 border-b border-gray-50 text-right">' +
          '<div class="font-bold text-gray-800 inline-flex items-center justify-end gap-2">' + formatMoney(item.a) + '<i class="fas fa-pen text-[9px] text-blue-400"></i></div>' +
  ...
  renderTableSafe('table-med', 'med', typeof cesGetTeamColor==='function'?cesGetTeamColor('MED'):'#004aad');
  renderTableSafe('table-lab', 'lab', typeof cesGetTeamColor==='function'?cesGetTeamColor('LAB'):'#19a7ce');
  renderTableSafe('table-ehs', 'ehs', typeof cesGetTeamColor==='function'?cesGetTeamColor('EHS'):'#0fc1a1');
}

function openRevenueEditModal(team, preferredMonth) {
  ...
  const colors = { med: cesGetTeamColor('MED'), lab: cesGetTeamColor('LAB'), ehs: cesGetTeamColor('EHS') };
  ...
  if (preferredMonth && Array.from(monthSelect.options).some(function(option){ return option.value === String(preferredMonth); })) {
    monthSelect.value = String(preferredMonth);
  }
  monthSelect.onchange();
  modal.classList.remove('hidden');
  setTimeout(function(){ const input=document.getElementById('edit-rev-actual'); if(input){ input.focus(); input.select(); } }, 80);
}
...
function renderRevenueChart(chartData) {
  ...
  revenueChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { label: 'MED', data: chartData.med_a || [], backgroundColor: (typeof cesGetTeamColor==='function'?cesGetTeamColor('MED'):'#004aad'), targetData: chartData.med_t || [], barPercentage: 0.6, categoryPercentage: 0.8 },
        { label: 'LAB', data: chartData.lab_a || [], backgroundColor: (typeof cesGetTeamColor==='function'?cesGetTeamColor('LAB'):'#19a7ce'), targetData: chartData.lab_t || [], barPercentage: 0.6, categoryPercentage: 0.8 },
        { label: 'EHS', data: chartData.ehs_a || [], backgroundColor: (typeof cesGetTeamColor==='function'?cesGetTeamColor('EHS'):'#0fc1a1'), targetData: chartData.ehs_t || [], barPercentage: 0.6, categoryPercentage: 0.8 }
      ]
    },
    options: {
      responsive: true,
