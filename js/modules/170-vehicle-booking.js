...
function cesBookingInjectStyles_() {
  ...
  style.textContent = [
    ...
    '#view-car_booking .vehicle-calendar-day.ces-booking-past-v212.ces-booking-past-booked{background:#fee2e2!important;border-color:#f87171!important;color:#991b1b!important;opacity:1!important;filter:none!important}',
    '#view-car_booking .vehicle-calendar-day.ces-booking-past-booked .vehicle-calendar-state{color:#b91c1c!important;font-weight:900}',
...
function renderVehicleCalendar_(type) {
  ...
  for(var i=0;i<42;i++){
    ...
    var tooltip=isPast?(rows.length?'Past date · '+rows.length+' booking record(s)':'Past date — booking is disabled'):status.intervals.map(function(item){return cesBookingMinuteText_(item.start)+'–'+cesBookingMinuteText_(item.end)+' '+(item.row.jobTitle||item.row.destination||item.row.bookingId||'');}).join('\n');
    var clickable=type==='CAR'&&date.getMonth()===month.getMonth()&&!isPast;
    html+='<div class="vehicle-calendar-day '+status.css+muted+(isPast?' ces-booking-past-v212':'')+(isPast&&rows.length?' ces-booking-past-booked':'')+'" data-date="'+key+'" aria-disabled="'+(isPast?'true':'false')+'" title="'+cesBookEsc_(tooltip)+'" '+(clickable?'onclick="showVehicleDaySlots(\'CAR\',\''+key+'\')"':'')+'><div class="vehicle-calendar-number">'+date.getDate()+'</div>'+(rows.length?'<span class="vehicle-calendar-count">'+rows.length+'</span>':'')+'<div class="vehicle-calendar-state">'+(isPast?(rows.length?'Booked':'Past date'):status.label)+'</div></div>';
  }
  root.innerHTML=html;
}
...
function renderVanBookingDashboard_(result){
  ...
  root.innerHTML='<div class="overflow-auto rounded-xl border border-slate-200 max-h-[560px]"><table class="w-full min-w-[1320px] table-fixed text-xs text-left"><colgroup><col style="width:150px"><col style="width:80px"><col style="width:110px"><col style="width:520px"><col style="width:105px"><col style="width:110px"><col style="width:210px"></colgroup><thead class="sticky top-0 z-10 bg-slate-50 text-[9px] uppercase tracking-wider text-slate-500"><tr><th class="p-3">Date / Time</th><th class="p-3">Team</th><th class="p-3">Cost Center</th><th class="p-3">Booking / Route Details</th><th class="p-3">Usage</th><th class="p-3">Cost</th><th class="p-3">Traveller / Driver</th></tr></thead><tbody class="divide-y divide-slate-100">'+result.events.map(item=>{
    ...
    const route=item.route||item.location||item.description||item.detail||item.bookingDetails||'-';
...
function cesVanSnapshotKey_(year,month,team){return'CES_VAN_DASHBOARD_V35_'+[year,month,team].join('_');}
function cesVanReadSnapshot_(key){try{var raw=sessionStorage.getItem(key)||localStorage.getItem(key);return raw?JSON.parse(raw):null;}catch(ignore){return null;}}
function cesVanWriteSnapshot_(key,data){try{sessionStorage.setItem(key,JSON.stringify(data));localStorage.setItem(key,JSON.stringify(data));}catch(ignore){}}
async function loadVanBookingDashboard(force){
  ...
  const snapshotKey=cesVanSnapshotKey_(year,month,team),snapshot=cesVanReadSnapshot_(snapshotKey);
  ...
  const task=(async()=>{
    try{
      const result=await window.CES_API.callFunction('getVanBookingDashboard',[year,month,team,!!force],{transport:'jsonp',timeoutMs:30000,dedupe:!force,priority:'active',module:'van_booking'});
      ...
      cesVanWriteSnapshot_(snapshotKey,result);
