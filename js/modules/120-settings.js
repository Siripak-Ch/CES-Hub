...
// --- Setting Module Logic ---
    ...
    function onSettingsLoaded(data) {
        ...
        try { if (data.ROLE_PERMISSIONS && typeof window.updatePermissionSummary === 'function') window.updatePermissionSummary(JSON.parse(data.ROLE_PERMISSIONS)); } catch(ignorePerm) {}
        ...
        setVal('cfg-memo-workorder-sheet-id', data.MEMO_WORKORDER_SOURCE_SPREADSHEET_ID || '');
        ...
        setVal('cfg-link-ces-home', data.LINK_CES_HUB_HOME || ''); setVal('cfg-link-infusion-dashboard', data.LINK_INFUSION_PUMP_DASHBOARD || ''); setVal('cfg-link-rental-contact', data.LINK_RENTAL_CONTACT_PAGE || ''); setVal('cfg-link-master-cal-pm', data.LINK_MASTER_CAL_PM_SOURCE || ''); setVal('cfg-link-inventory-source', data.LINK_INVENTORY_SOURCE || ''); setVal('cfg-link-check-stock', data.LINK_CHECK_STOCK || '');
    ...
    function collectFullSystemConfig_() {
        ...
        return {
            ...
            LINK_KPI_EHS_SHEET:getVal('cfg-link-kpi-ehs'), LINK_KPI_LAB_SHEET:getVal('cfg-link-kpi-lab'), LINK_MEMO_WORKORDER_SOURCE:getVal('cfg-link-memo-workorder'), MEMO_WORKORDER_SOURCE_SPREADSHEET_ID:getVal('cfg-memo-workorder-sheet-id'), LINK_TRAINING_PLAN_2026:getVal('cfg-link-training-plan'),
            ...
            LINK_CES_HUB_HOME:getVal('cfg-link-ces-home'), LINK_INFUSION_PUMP_DASHBOARD:getVal('cfg-link-infusion-dashboard'), LINK_RENTAL_CONTACT_PAGE:getVal('cfg-link-rental-contact'), LINK_MASTER_CAL_PM_SOURCE:getVal('cfg-link-master-cal-pm'), LINK_INVENTORY_SOURCE:getVal('cfg-link-inventory-source'), LINK_CHECK_STOCK:getVal('cfg-link-check-stock'),
    ...
    const CES_SETTING_SECTION_KEYS = {
        ...
        links:['LINK_SERVICE_CSI_CES_SUMMARY','LINK_SERVICE_CSI_TES_SUMMARY','LINK_REPORT_CSI_SUMMARY','LINK_REVENUE_DASHBOARD','LINK_KPI_EHS_SHEET','LINK_KPI_LAB_SHEET','LINK_MEMO_WORKORDER_SOURCE','MEMO_WORKORDER_SOURCE_SPREADSHEET_ID','LINK_TRAINING_PLAN_2026','KPI_DRIVE_MED','KPI_DRIVE_LAB','KPI_DRIVE_EHS','KPI_SHEET_MED','KPI_SHEET_LAB','KPI_SHEET_EHS','LINK_CES_HUB_HOME','LINK_INFUSION_PUMP_DASHBOARD','LINK_RENTAL_CONTACT_PAGE','LINK_MASTER_CAL_PM_SOURCE','LINK_INVENTORY_SOURCE','LINK_CHECK_STOCK','AUDIT_DRIVE_MED','AUDIT_EXCEL_MED','AUDIT_DRIVE_LAB','AUDIT_EXCEL_LAB','AUDIT_DRIVE_EHS','AUDIT_EXCEL_EHS'],
        ...
        modulelinks:['LINK_CES_HUB_HOME','LINK_INFUSION_PUMP_DASHBOARD','LINK_RENTAL_CONTACT_PAGE','LINK_MASTER_CAL_PM_SOURCE','LINK_INVENTORY_SOURCE','LINK_CHECK_STOCK','AUDIT_DRIVE_MED','AUDIT_EXCEL_MED','AUDIT_DRIVE_LAB','AUDIT_EXCEL_LAB','AUDIT_DRIVE_EHS','AUDIT_EXCEL_EHS']
    };

    async function saveSettingSection(section, button) {
        const keys = CES_SETTING_SECTION_KEYS[String(section || '').toLowerCase()] || [];
        if (!keys.length) return;
        const all = collectFullSystemConfig_(), patch = {CONFIG_SCHEMA_VERSION:'30.0.37'};
...
function renderPortalLinksSetting(){
  ...
  const rows=(CES_PORTAL_LINKS_SETTING||[]).slice().sort((a,b)=>(sectionRank[String(a.section||'').toUpperCase()]||9)-(sectionRank[String(b.section||'').toUpperCase()]||9)||Number(a.sortOrder||999)-Number(b.sortOrder||999));
