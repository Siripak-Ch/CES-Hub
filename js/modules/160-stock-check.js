...
function sc_lookup(){
  ...
  google.script.run.withSuccessHandler(res=>{
    ...
    if((res.data||[]).length===1) setTimeout(()=>sc_openResultPopup(res.data[0]),30);
    sc_loadLogs();
...
function sc_renderResult(rows){
  ...
  spSetHtml('scResult',rows.map(d=>`<div class="stockpro-card sc-result-compact" onclick='sc_openResultPopup(${JSON.stringify(d).replace(/'/g,"&#39;")})' style="cursor:pointer"><div class="stockpro-card-head"><h3>${spEsc(d.idCode)} ${spBadge(d.status)}</h3><span class="sp-pill">${spEsc(d.model||d.itemName||'-')}</span></div><div class="sp-muted" style="margin-top:6px">SN ${spEsc(d.sn||d.serialNumber||'-')} · กดเพื่อดู Action</div><div class="sp-muted" style="margin-top:8px">กดเพื่อดูรายละเอียดและ Action</div></div>`).join(''));
}
function sc_openResultPopup(d){
  ...
  const effectiveStatus=(status||rental||'').trim();
  const model=d.model||d.itemName||d.name||'-';
  const rentalFlow=/เช่า|ยืม|overdue|in[ -]?use/i.test(effectiveStatus);
  const overdueFlow=/overdue|เกินกำหนด/i.test(effectiveStatus);
  const calibrationFlow=/รอสอบเทียบ|recheck|calibration/i.test(status);
  const readyFlow=/พร้อมส่ง|ready|stock/i.test(status);
  const brokenFlow=/ใช้งานไม่ได้|broken|damaged|defect|ชำรุด|เสีย/i.test(status);
  const missingFlow=/ไม่พบในรายการ|missing|lost|not found|สูญหาย/i.test(status);
  const detailHtml='<div class="sc-popup-grid sc-popup-grid-minimal">'+field('Serial ID',d.sn||d.serialNumber)+field('ID Code',d.idCode||d.id)+field('Name',model)+field('Status',effectiveStatus||'-')+'</div>';
  if(SC.mode==='CAL/PM'){
    if(!calibrationFlow){Swal.fire({title:(id||'-')+' · '+spEsc(model),html:detailHtml+'<div class="sp-muted" style="margin-top:12px">'+(readyFlow?'สอบเทียบแล้ว / พร้อมส่ง':'เปลี่ยนสถานะไม่ได้ ต้องเป็น รอสอบเทียบ เท่านั้น')+'</div>',icon:readyFlow?'success':'info',confirmButtonText:'ปิด'});return;}
    Swal.fire({title:(id||'-')+' · '+spEsc(model),html:detailHtml+'<div class="sp-muted" style="margin-top:12px">รอสอบเทียบ → พร้อมส่ง</div>',icon:'question',showCancelButton:true,reverseButtons:true,cancelButtonText:'ยกเลิก',confirmButtonText:'ยืนยันสอบเทียบพร้อมส่ง',confirmButtonColor:'#2563eb'}).then(function(r){if(r.isConfirmed)sc_record(id,'CALIBRATION_CONFIRM');});return;
  }
  const actions=[];
  if(rentalFlow||overdueFlow){actions.push(`<button class=\"sc-white-action green\" onclick=\"sc_record('${id}','CHECK-IN');Swal.close()\"><i class=\"fas fa-rotate-left\"></i> รับคืน</button>`);}
  ...
  if(brokenFlow||missingFlow){actions.push(`<button class=\"sc-white-action green\" onclick=\"sc_record('${id}','RECOVER');Swal.close()\"><i class=\"fas fa-rotate-left\"></i> พบแล้ว / ส่งสอบเทียบ</button>`);}
  if(!brokenFlow&&!missingFlow){actions.push(`<button class=\"sc-white-action orange\" onclick=\"sc_record('${id}','MARK_BROKEN');Swal.close()\"><i class=\"fas fa-screwdriver-wrench\"></i> ใช้งานไม่ได้</button>`);actions.push(`<button class=\"sc-white-action red\" onclick=\"sc_record('${id}','MARK_MISSING');Swal.close()\"><i class=\"fas fa-eye-slash\"></i> ไม่พบในรายการ</button>`);}
  Swal.fire({title:`${id||'-'} · ${spEsc(model)}`,width:'min(620px,94vw)',html:`<div class=\"sc-popup-grid sc-popup-grid-minimal\">${field('Serial ID',d.sn||d.serialNumber)}${field('ID Code',d.idCode||d.id)}${field('Name',model)}${field('Status',effectiveStatus||'-')}</div><div class=\"sc-popup-actions\">${actions.join('')}</div>`,showConfirmButton:false,showCloseButton:true});
}
...
(function(){if(document.getElementById('ces-v35-check-style'))return;var st=document.createElement('style');st.id='ces-v35-check-style';st.textContent='.sc-white-action,.sc-white-action i{color:#fff!important}@media(max-width:600px){#view-check_stock .sc-result-summary{display:none!important}}';document.head.appendChild(st)})();
