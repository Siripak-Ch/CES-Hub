...
function displayDate(v){var s=String(v||'').trim();if(!s)return'-';var m=s.match(/^(\d{4})[-\/.](\d{1,2})[-\/.](\d{1,2})/);if(m)return String(m[3]).padStart(2,'0')+'/'+String(m[2]).padStart(2,'0')+'/'+m[1];m=s.match(/^(\d{1,2})[-\/.](\d{1,2})[-\/.](\d{4})/);if(m)return String(m[1]).padStart(2,'0')+'/'+String(m[2]).padStart(2,'0')+'/'+m[3];var d=new Date(s);return isNaN(d.getTime())?s:String(d.getDate()).padStart(2,'0')+'/'+String(d.getMonth()+1).padStart(2,'0')+'/'+d.getFullYear();}
...
async function handleMemoWorkOrderUpload(file){
  ...
  try{Swal.fire({title:'Reading Memo / Work Order...',html:'<div class="text-xs text-slate-500">กำลังอ่านเฉพาะ 3 เดือนล่าสุดใน background เพื่อไม่ให้หน้าเว็บค้าง</div>',allowOutsideClick:false,showConfirmButton:false,didOpen:function(){Swal.showLoading();}});
    ...
    state.page=1;await load(true);
    var rematch=await syncMemoWorkOrderServiceCSI_(result,parsed.memoRows,parsed.workOrderRows);
    Swal.fire({icon:'success',title:'Import & rematch complete',html:'3 เดือนล่าสุด · Memo <b>'+Number(result.memoAdded||0)+'</b> added · Work Order <b>'+Number(result.workOrderAdded||0)+'</b> added · Duplicate <b>'+Number(result.duplicate||0)+'</b><br><span class="text-[10px] text-slate-500">Service CSI 2026: '+esc(rematch&&rematch.message||'rematch checked')+'</span>',confirmButtonColor:'#003DA5'});
...
async function syncMemoWorkOrderServiceCSI_(result,memoRows,workOrderRows){
  try{if(!result||!result.uploadId||!window.CES_API)return{success:false,message:'No upload ID for rematch'};var months=[].concat(memoRows||[],workOrderRows||[]).map(function(r){return String(r.date||'').slice(0,7);}).filter(function(m){return /^2026-\d{2}$/.test(m);});months=[...new Set(months)];if(!months.length)return{success:true,message:'No 2026 month requires rematch'};
    var svc=await window.CES_API.callFunction('getServiceDataForMemoSync',[{months:months}],{transport:'jsonp',timeoutMs:90000,dedupe:false,priority:'active',userAction:true,module:'memo_workorder'}),csiRows=Array.isArray(svc&&svc.rows)?svc.rows:[];if(!csiRows.length)return{success:true,message:'No Service CSI rows in '+months.join(', ')};if(typeof window.svcBuildMemoMappingV55_!=='function')return{success:false,message:'Matching module is unavailable'};
    var previous=window.serviceRawData,mapping=[];try{window.serviceRawData=csiRows;mapping=window.svcBuildMemoMappingV55_(memoRows.map(function(r){return Object.assign({type:'MEMO'},r);}),workOrderRows.map(function(r){return Object.assign({type:'WORK_ORDER'},r);}));}finally{window.serviceRawData=previous;}
    if(!mapping.length)return{success:true,message:'0 matching rows generated'};var compact=typeof window.svcMapV55CompactRow_==='function'?mapping.map(window.svcMapV55CompactRow_):mapping;
    var saved=await window.CES_API.callFunction('saveServiceMemoMapping',[{uploadId:result.uploadId,sourceFile:result.sourceFile||'',uploadedAt:new Date().toISOString(),targetYear:2026,importMonths:months,memoRows:memoRows,workOrderRows:workOrderRows,mappingRows:compact}],{transport:'iframe',timeoutMs:240000,dedupe:false,priority:'active',userAction:true,module:'memo_workorder'});
    if(!saved||saved.success===false)throw new Error((saved&&saved.message)||'Rematch save failed');
    try{if(typeof window.refreshServiceMemoMappingV55==='function')await window.refreshServiceMemoMappingV55();}catch(ignoreRefresh){}
    return{success:true,message:Number(saved.matchedRows||0)+' matched · '+Number(saved.unmatchedRows||0)+' unmatched'};
  }catch(e){console.warn('[Memo → Service CSI] rematch failed',e);return{success:false,message:e.message||String(e)};}
}
