...
function recent3(v){var m=String(v||'').match(/^(\\d{4})[-\\/](\\d{1,2})/);if(!m)return false;var now=new Date(),cut=new Date(now.getFullYear(),now.getMonth()-2,1),d=new Date(Number(m[1]),Number(m[2])-1,1);return d.getTime()>=cut.getTime();}
