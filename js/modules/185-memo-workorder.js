...
async function handleMemoWorkOrderUpload(file){if(!file)return;if(!window.XLSX){Swal.fire('Upload','XLSX library is not ready. Refresh and try again.','error');return;}try{Swal.fire({title:'Reading Memo / Work Order...',allowOutsideClick:false,showConfirmButton:false,didOpen:function(){Swal.showLoading();}});var wb=XLSX.read(await file.arrayBuffer(),{type:'array',cellDates:true});var woRaw=rowsFromSheet(wb.Sheets.WorkOrder),memoRaw=rowsFromSheet(wb.Sheets.Memo);if(!memoRaw.length&&!woRaw.length)throw new Error('ไม่พบ Sheet Memo หรือ WorkOrder ในไฟล์');var teamByForm={};woRaw.forEach(function(r){var form=String(pick(r,['FormID','Form ID'])||'').trim(),team=normalizeTeam(pick(r,['Team2','Team']));if(form&&team)teamByForm[form]=team;});var workOrderRowsAll=woRaw.map(function(r,i){var form=String(pick(r,['FormID','Form ID'])||'').trim();return{sourceRow:i+2,formId:form,docNo:String(pick(r,['Job No.','Job No','Work Order No.','WorkOrder No.'])||'').trim(),date:excelDate(pick(r,['Start Date/Time','Date','Start Date'])),endDate:excelDate(pick(r,['End Date/Time','End Date'])),customer:String(pick(r,['Customer Name','Customer'])||'').trim(),team:normalizeTeam(pick(r,['Team2','Team'])),url:String(pick(r,['WorkOrderURL','Work Order URL','URL'])||'').trim()};}).filter(function(r){return(r.formId||r.docNo||r.customer)&&memoWorkOrderRecent3Months_(r.date);});var memoRowsAll=memoRaw.map(function(r,i){var form=String(pick(r,['FormID','Form ID'])||'').trim();return{sourceRow:i+2,formId:form,docNo:String(pick(r,['Memo No.','MemoNo','Memo No'])||'').trim(),date:excelDate(pick(r,['Start Date','วันที่','Date'])),endDate:excelDate(pick(r,['End Date'])),customer:String(pick(r,['Customer Name','Customer'])||'').trim(),team:normalizeTeam(pick(r,['Team'])||teamByForm[form]||''),url:String(pick(r,['MemoOrderURL','Memo URL','URL'])||'').trim()};}).filter(function(r){return(r.formId||r.docNo||r.customer)&&memoWorkOrderRecent3Months_(r.date);});if(!memoRowsAll.length&&!workOrderRowsAll.length)throw new Error('ไม่พบข้อมูล Memo / Work Order ในช่วง 3 เดือนล่าสุด');var result=await window.CES_API.callFunction('saveMemoWorkOrderImport',[{sourceFile:file.name,uploadedAt:new Date().toISOString(),memoRows:memoRowsAll,workOrderRows:workOrderRowsAll}],{transport:'iframe',timeoutMs:120000,dedupe:false});if(!result||result.success===false)throw new Error((result&&result.message)||'Upload failed');var mappingStatus='';
      try{
        if(result.mappingSyncRecommended&&typeof svcBuildMemoMappingV55_==='function'){
          if((typeof serviceRawData==='undefined'||!Array.isArray(serviceRawData)||!serviceRawData.length)&&window.CES_API&&typeof window.CES_API.callFunction==='function'){
            try{var svcPayload=await window.CES_API.callFunction('getServiceDataOnly',[],{transport:'jsonp',timeoutMs:120000,dedupe:true,priority:'active',module:'service'});if(Array.isArray(svcPayload))serviceRawData=svcPayload;}catch(ignoreServiceLoad){console.warn('[Memo → Service CSI] Service CSI data preload failed',ignoreServiceLoad);}
          }
          if(typeof serviceRawData==='undefined'||!Array.isArray(serviceRawData)||!serviceRawData.length)throw new Error('Service CSI 2026 data is not loaded. Import completed; mapping sync will remain pending.');
          var memoForMap=memoRowsAll.map(function(r){return Object.assign({type:'MEMO'},r);});
          var woForMap=workOrderRowsAll.map(function(r){return Object.assign({type:'WORK_ORDER'},r);});
          var mappingRows=svcBuildMemoMappingV55_(memoForMap,woForMap);
          var importMonths=[...new Set(memoForMap.concat(woForMap).map(function(r){return String(r.date||'').slice(0,7);}).filter(function(m){return /^2026-\d{2}$/.test(m);} ))];
          if(mappingRows.length&&importMonths.length&&window.CES_API&&typeof window.CES_API.callFunction==='function'){
            var mapSaved=await window.CES_API.callFunction('saveServiceMemoMapping',[{uploadId:result.uploadId||('MWO-'+Date.now()),sourceFile:file.name,uploadedAt:new Date().toISOString(),targetYear:2026,importMonths:importMonths,memoRows:memoForMap,workOrderRows:woForMap,mappingRows:mappingRows.map(typeof svcMapV55CompactRow_==='function'?svcMapV55CompactRow_:function(x){return x;})}],{transport:'iframe',timeoutMs:180000,dedupe:false,priority:'active',userAction:true,module:'service'});
            if(mapSaved&&mapSaved.success)mappingStatus=' · CSI mapping synced '+Number(mapSaved.matchedRows||0)+' matched';
          }
        }
      }catch(mappingError){console.warn('[Memo → Service CSI sync]',mappingError);mappingStatus=' · CSI mapping sync pending';}
      Swal.fire({icon:'success',title:'Import complete',html:'3 เดือนล่าสุด · Memo <b>'+Number(result.memoAdded||0)+'</b> added · Work Order <b>'+Number(result.workOrderAdded||0)+'</b> added · Duplicate <b>'+Number(result.duplicate||0)+'</b>'+mappingStatus,confirmButtonColor:'#003DA5'});state.page=1;await load(true);}catch(e){Swal.fire('Upload Error',e.message||String(e),'error');}}
...
window.handleMemoWorkOrderUpload=handleMemoWorkOrderUpload;
