...
// ==========================================
    ...
    function parseMedWorkbook_(workbook){
      const preferredName=workbook.Sheets['MED Fast Import']?'MED Fast Import':(workbook.Sheets['Master Plan']?'Master Plan':(workbook.Sheets['Calendar Upload']?'Calendar Upload':(workbook.Sheets['SEP-OCT Data']?'SEP-OCT Data':workbook.SheetNames[0])));const sheet=workbook.Sheets[preferredName];if(!sheet)return[];
...
/* V30.0.43 — MED Monthly Work Plan import reliability. */
(function(w,d){var base=w.handleMedPlanFile;if(typeof base!=='function')return;w.handleMedPlanFile=async function(input){var file=input&&input.files?input.files[0]:null;if(!file)return;try{if(!w.XLSX)throw new Error('XLSX library is not loaded.');if(w.Swal)w.Swal.fire({title:'Reading MED Monthly Work Plan…',html:'ตรวจสอบ Master Plan / Calendar Upload / SEP-OCT Data',allowOutsideClick:false,showConfirmButton:false,didOpen:function(){w.Swal.showLoading();}});var workbook=w.XLSX.read(await file.arrayBuffer(),{type:'array',cellDates:true}),rows=typeof parseMedWorkbook_==='function'?parseMedWorkbook_(workbook):[];if(!rows.length)throw new Error('No MED weekly plan rows were found in '+file.name+'.');if(w.Swal)w.Swal.close();var sample=rows.slice(0,10).map(function(r){return'<tr><td class="p-2">'+calendarEsc_(r.startDate)+' → '+calendarEsc_(r.endDate)+'</td><td class="p-2">'+calendarEsc_(r.teamGroup)+'</td><td class="p-2">'+calendarEsc_(r.title)+'</td><td class="p-2">'+calendarEsc_(r.engineer||'-')+'</td></tr>';}).join('');var answer=await w.Swal.fire({title:'Import MED Monthly Work Plan Sep–Oct 2026?',html:'<div class="text-left text-xs"><div class="mb-3"><b>'+rows.length+'</b> work-plan blocks found in <b>'+calendarEsc_(file.name)+'</b>.</div><div class="max-h-72 overflow-auto border rounded-xl"><table class="w-full"><thead class="bg-slate-50"><tr><th class="p-2">Date range</th><th class="p-2">Team</th><th class="p-2">Work plan</th><th class="p-2">Engineer</th></tr></thead><tbody>'+sample+'</tbody></table></div><p class="mt-3 text-slate-500">Existing manual MED Calendar events are kept; only CES-imported MED work-plan events are synchronized.</p></div>',icon:'question',showCancelButton:true,confirmButtonText:'Import to MED Calendar',confirmButtonColor:'#004aad',width:820});if(!answer.isConfirmed)return;if(w.Swal)w.Swal.fire({title:'Importing MED calendar…',html:'กำลัง Sync เฉพาะ MED work-plan events',allowOutsideClick:false,showConfirmButton:false,didOpen:function(){w.Swal.showLoading();}});var result=await w.CES_API.callFunction('importMedMonthlyWorkPlan',[{rows:rows,sourceFile:file.name},{syncDashboard:false,hardReplace:false}],{transport:'iframe',timeoutMs:300000,priority:'user',userAction:true,module:'calendar',dedupe:false});if(!result||(!result.success&&Number(result.created||0)===0))throw new Error((result&&result.message)||'MED import failed.');if(w.Swal)w.Swal.close();var warning=Array.isArray(result.errors)?result.errors:[];await w.Swal.fire({title:result.partial?'MED Import Complete with Warnings':'MED Import Complete',html:'<div class="text-left text-sm"><p><b>'+Number(result.created||0)+'</b> created · <b>'+Number(result.skippedExisting||0)+'</b> unchanged · <b>'+Number(result.deleted||0)+'</b> removed · <b>'+Number(result.validRows||rows.length)+'</b> source blocks.</p>'+(warning.length?'<div class="mt-3 text-amber-700"><b>Warnings</b><ul class="list-disc pl-5">'+warning.slice(0,8).map(function(x){return'<li>'+calendarEsc_(x)+'</li>';}).join('')+'</ul></div>':'')+'</div>',icon:result.partial?'warning':'success',confirmButtonColor:'#004aad',width:720});if(typeof w.cesRefreshCalendar_==='function')await w.cesRefreshCalendar_();else if(typeof w.loadAllData==='function')await w.loadAllData(true);}catch(e){if(w.Swal)w.Swal.fire({title:'MED Import Error',html:'<div class="text-left text-sm">'+calendarEsc_(e&&e.message||e)+'</div>',icon:'error',width:720,confirmButtonColor:'#004aad'});}finally{if(input)input.value='';}};})(window,document);
