...
// ============================================================
    ...
    function cesRunTabInit_(tab){
        ...
        else if (tab === 'master_cal_pm_plan' && typeof window.initMasterCalPmPlan === 'function') window.initMasterCalPmPlan(forceRefresh);
        ...
        else if (tab === 'report_manage') { if (typeof initOTGenerate === 'function') initOTGenerate(); else if (typeof initReportManage === 'function') initReportManage(); }
    ...
    const CES_VALID_TABS_V60 = ['portal','management_overview','yearly','revenue','ot','service','report','memo_workorder','calendar','checkin','car_booking','van_booking','weekly','report_manage','kpi','stock_dashboard','inventory','check_stock','team_information','team_plan','master_cal_pm_plan','monthly_report','users','ces_evaluation','ces_ai_knowledge','setting','health'];
    ...
    function cesForceAdminSystemMenu_() {
        ...
        ['setting','users','ces_evaluation','ces_ai_knowledge','health','team_information','team_plan','master_cal_pm_plan','monthly_report'].forEach(function(id){
    ...
    function applyRolePermissions(role) {
        ...
        if (role === 'ADMIN') {
            ...
            if (role === 'STAFF') {
                ['portal','management_overview','checkin','service','memo_workorder','ot','calendar','car_booking','van_booking','weekly','report_manage','kpi','team_information','team_plan','master_cal_pm_plan','monthly_report'].forEach(t => {
                    const btn = document.getElementById('btn-' + t);
                    if (btn) btn.classList.remove('hidden');
                });
            }
            if (role === 'MANAGER') {
                ['portal','management_overview','yearly','revenue','ot','service','report','memo_workorder','calendar','checkin','car_booking','van_booking','weekly','report_manage','kpi','stock_dashboard','inventory','check_stock','team_information','team_plan','master_cal_pm_plan','monthly_report','ces_evaluation'].forEach(t => {
                    const btn = document.getElementById('btn-' + t);
                    if (btn) btn.classList.remove('hidden');
                });
            }
            if (role === 'SUPERVISOR') {
                ['portal','management_overview','checkin','service','report','memo_workorder','ot','calendar','car_booking','van_booking','yearly','revenue','weekly','report_manage','kpi','stock_dashboard','inventory','check_stock','team_information','team_plan','master_cal_pm_plan','monthly_report'].forEach(t => {
                    const btn = document.getElementById('btn-' + t);
                    if (btn) btn.classList.remove('hidden');
                });
            }
    ...
    function switchTab(tab) {
        ...
        const views = ['portal','management_overview','service','report','memo_workorder','yearly','revenue','calendar','checkin','car_booking','van_booking','team_information','team_plan','master_cal_pm_plan','monthly_report','users','ces_evaluation','ces_ai_knowledge','setting','health','ot','weekly','report_manage','kpi','stock_dashboard','inventory','check_stock'];
        views.forEach(v => {
        ...
        const titles = {
            ...
            car_booking: 'Car Booking', van_booking: 'Van Booking', team_information: 'Team Information', team_plan:'Team Plan', master_cal_pm_plan:'Master CAL/PM Plan', monthly_report:'Monthly Report',
    ...
    function getRequestedTabFromUrl() {
        try {
            ...
            const alias = {
                ...
                'team-plan': 'team_plan',
                team_plan: 'team_plan',
                'master-cal-pm-plan': 'master_cal_pm_plan',
                master_cal_pm: 'master_cal_pm_plan',
                ...
                'ot-generate': 'report_manage',
                ot_generate: 'report_manage',
    ...
    function loadAllData(options) {
        ...
        if (loadingText) {
            const titleMap = {
                ...
                inventory:'Inventory', check_stock:'Check Stock', team_information:'Team Information', team_plan:'Team Plan', master_cal_pm_plan:'Master CAL/PM Plan', monthly_report:'Monthly Report', ces_evaluation:'CES Hub Evaluation', ces_ai_knowledge:'CES AI Knowledge',
